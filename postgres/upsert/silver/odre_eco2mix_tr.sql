-- =============================================================================
-- UPSERT for silver.fact_eco2mix_tr
-- Source: Silver odre_eco2mix_tr parquet
-- Strategy: COPY to staging + UPSERT to target
--
-- Note: For large fact tables like this one (~400k rows), a TRUNCATE + COPY
-- strategy could be faster if full refresh is acceptable. This would skip
-- the staging table and conflict checking entirely.
-- =============================================================================
-- @target_table: silver.fact_eco2mix_tr
-- @primary_key: code_insee_region, date_heure

INSERT INTO silver.fact_eco2mix_tr (
...
ON CONFLICT (code_insee_region, date_heure) DO UPDATE SET
    ...
    updated_at = NOW()
WHERE (
    silver.fact_eco2mix_tr.consommation,
    silver.fact_eco2mix_tr.solaire,
    silver.fact_eco2mix_tr.eolien
) IS DISTINCT FROM (
